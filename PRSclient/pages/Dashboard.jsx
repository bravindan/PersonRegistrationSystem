...
useEffect(() => {
  ...
  return (
    <div className="p-3">
        ...
        <>
          ...
          <div className='overflow-x-auto'>
            <table className="w-full border-collapse border text-sm">
              ...
              <tbody>
                {people.length==0? (<tr className=''>
                  <td colSpan={15} className='text-xl text-red-600 text-center'>No records found</td>
                  </tr>)
                   :
                    ...
                    <td className="p-2">{person.createdById == 1? "Bravin": "Dante"}</td>
      ...
      Supervised Person Data
      </p>
      {approved.length ==0 ? "No data records found" :
        <>
          <div className='flex justify-between mb-4'>
          </div>
          <div className='overflow-x-auto'>
              ...
              <tbody>
                {approved.map((person, index) => (
                  <tr key={person.id} className={(person.id % 2 === 0) ? 'bg-gray-100 text-gray-700' : 'bg-white text-gray-700'}>
                    ...
                    <td className="p-2">{formatDate(person.createdOn)}</td>
                    {/* <td className="p-2">{resolveStatus(person.approvalStatus)}</td>  */}
                    <td className='p-2'>{person.supervisedById == 3? "Kitush": "Dave"}</td>
                    ...
                    <td className="p-2">
                      <div className='flex justify-between'>
                        {userType == "1" && (
                          ...
                          <div className='flex'>
                            <button onClick={() => navigateToEditApproved(person.id)} className="text-white px-2 py-1 rounded bg-blue-600"> Edit</button>
                          </div>
                        )}
                        {
                          userType =="0" && (
                            <div className='flex'>
                            {/* <button onClick={() => navigateToEdit(person.id)} className="text-white px-2 py-1 rounded bg-blue-600"> Edit</button> */}
