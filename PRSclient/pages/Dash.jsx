...
export default function Dash() {
  ...
  const DeletePerson = async (id) => {
    try {
      ...
      alert(`Person with id: ${id} removed`);
    } catch (error) {
      alert("Could not delete person", error);
    }
  }
  ...
  const handleRejection = async (id) => {
    ...
    await axios.delete(`https://localhost:7057/api/Person/reject`, personDTO);
    getData();
  }
                    ...
                    {userType == 1 && (
                      <td className="p-2">
                        <div className='flex justify-between'>
                          ...
                          <button onClick={() => handleRejection(person.id)} className={`${person.approvalStatus === 2 ? 'bg-red-500' : 'bg-black'} text-white px-2 py-1 rounded hover:bg-red-600`}>{person.approvalStatus === 2 ? "Rejected" : "Reject"}</button>
                        </div>
                      </td>
                    )}
                    {userType == 0 && (
